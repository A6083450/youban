...
const agents = computed<Array<{ id: AdminSkillAgentId, label: string }>>(() => (
  ['parent-assistant', 'destination-researcher', 'segment-planner', 'summary', 'itinerary-reviewer', 'plan-editor'] as const
).map(id => ({ id, label: t(`admin.skills.agents.${id}.name`) })))
...
function skillDisplayName(skill: AdminSkillSummary): string {
  if (skill.kind !== 'builtin')
    return skill.name
  const key = `admin.skills.builtins.${skill.name}.name`
  const localized = t(key)
  return localized === key ? skill.name : localized
}

function skillDisplayDescription(skill: AdminSkillSummary): string {
  if (skill.kind !== 'builtin')
    return skill.description
  const key = `admin.skills.builtins.${skill.name}.description`
  const localized = t(key)
  return localized === key ? skill.description : localized
}
...
<template>
  <section class="skills-panel">
    <view class="panel-heading">
      <view><text class="panel-eyebrow">{{ t('admin.skills.eyebrow') }}</text><text class="panel-title">{{ t('admin.skills.title') }}</text></view><view class="heading-actions">
    ...
    <view class="skills-layout">
      <aside class="skills-list">
        ...
        <button v-for="skill in skills" v-else :key="skill.id" :class="{ active: selected?.id === skill.id }" @click="selectSkill(skill.id)">
          <view><strong>{{ skillDisplayName(skill) }}</strong><text :data-state="skill.state">{{ t(`admin.skills.states.${skill.state}`) }}</text></view><text>{{ skillDisplayDescription(skill) }}</text><small>{{ t(`admin.skills.sources.${skill.source}`) }} · {{ t('admin.skills.list.assignedCount', { count: skill.agent_ids.length }) }}</small>
        </button>
      </aside>
      <view class="skill-detail">
        ...
        <template v-else>
          <view class="detail-heading">
            <view><text class="panel-eyebrow">{{ t(`admin.skills.sources.${selected.source}`) }}</text><text class="detail-title">{{ skillDisplayName(selected) }}</text><text>{{ skillDisplayDescription(selected) }}</text></view><button :disabled="saving || (selected.state !== 'archived' && selected.enabled)" @click="archiveOrRestore">
          ...
          <view class="detail-section">
            <view class="section-heading">
              <text class="section-label">SKILL.md</text><text>{{ t('admin.skills.versions.version', { number: selected.candidate_version?.version_number || selected.active_version?.version_number || 0 }) }}</text>
          ...
          <view class="version-list">
            <text class="section-label">{{ t('admin.skills.detail.versionsTitle') }}</text><view v-for="version in selected.versions" :key="version.id">
              <strong>{{ t('admin.skills.versions.version', { number: version.version_number }) }}</strong><text>{{ t(`admin.skills.versionStates.${version.state}`) }} · {{ version.created_at.replace('T', ' ').slice(0, 16) }}</text>
            </view>
          </view>
        </template>
      </view>
