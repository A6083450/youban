...
function attractionImage(item: TripAttraction): string {
  if (failedPhotoNames.value.has(item.name))
    return ''
  return resolveTripAttractionPhoto(item, props.attractionPhotos, getApiBaseUrl())
}

function onAttractionImageError(name: string): void {
          ...
          <view class="daily-facts">
            ...
            <view>
              ...
              <view class="daily-hotel-copy">
                ...
                <text v-if="item.day.hotel?.price_status === 'unavailable'">{{ t('result.hotelPriceUnavailable') }}</text>
              </view>
            </view>
          </view>
        </view>
          ...
          <view v-for="entry in timelineFor(item.day)" :key="entry.key" class="daily-timeline-item" :data-kind="entry.kind">
            ...
            <view class="daily-timeline-content">
              ...
              <template v-else-if="entry.kind === 'meal'">
                <text class="daily-entry-title"><wd-icon name="store" size="15px" /> {{ mealLabel(entry.item.type || entry.item.category || '') }}</text>
              ...
              <template v-else>
                ...
                <view class="daily-entry-details" :class="{ 'with-image': attractionImage(entry.item) }">
                  <image
                    v-if="attractionImage(entry.item)"
                    :src="attractionImage(entry.item)"
                    mode="aspectFill"
                    @error="onAttractionImageError(entry.item.name)"
                  />
