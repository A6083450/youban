...
type CompletedTripTaskStatus = Extract<TripTaskStatusResponseDto, { status: 'completed' }>
...
const progressStage = ref<TripTaskStageDto>('submitted')
const progressDetails = ref<TripTaskDetailDto[]>([])
...
function planPresentationVersion(status: CompletedTripTaskStatus): string {
  return `${status.plan_quality || 'legacy'}:${status.fast_plan_revision || 'none'}`
}
...
async function loadPlanPhotos(current: TripPlan, token: number): Promise<void> {
  const targets = collectTripPhotoTargets(current)
  const pending = targets.filter(target => !attractionPhotos.value[target.name])

  async function sweep(items: typeof pending): Promise<void> {
    let currentIndex = 0
    async function loadNext(): Promise<void> {
      while (currentIndex < items.length) {
        ...
        try {
          const source = await getPoiPhoto(target.name, target.city)
          if (source && token === photoGeneration) {
            attractionPhotos.value = {
              ...attractionPhotos.value,
              [target.name]: resolveMediaUrl(source, getApiBaseUrl()),
            }
          }
        }
        catch {
          // Keep the legacy text fallback when the POI provider has no usable photo.
        }
      }
    }
    await Promise.all(Array.from({ length: Math.min(2, items.length) }, () => loadNext()))
  }

  await sweep(pending)
  if (token !== photoGeneration)
    return
  const missing = pending.filter(target => !attractionPhotos.value[target.name])
  if (!missing.length)
    return
...
function refreshPlanPhotos(current: TripPlan): void {
  const token = ++photoGeneration
  const targetNames = new Set(collectTripPhotoTargets(current).map(target => target.name))
  attractionPhotos.value = Object.fromEntries(Object.entries(attractionPhotos.value).filter(([name]) => targetNames.has(name)))
  void loadPlanPhotos(current, token)
}
...
async function loadPlan(token: number): Promise<void> {
  try {
    ...
    if (status.status === 'completed') {
      ...
      const nextPlanVersion = planPresentationVersion(status)
      if (nextPlanVersion !== displayedPlanVersion) {
        plan.value = result
        refreshPlanPhotos(result)
        displayedPlanVersion = nextPlanVersion
      }
    ...
    progressDetails.value = status.details || []
...
async function exportGuide(format: 'image' | 'pdf' = 'image'): Promise<void> {
  ...
  try {
    ...
    const photographedPlan: TripPlan = {
      ...current,
      days: current.days.map(day => ({
        ...day,
        attractions: day.attractions.map(attraction => ({
          ...attraction,
          image_url: attraction.image_url || attractionPhotos.value[attraction.name],
        })),
      })),
    }
    const exportPlan: TripPlan = photographedPlan.budget || !budgetLedger.value
      ? photographedPlan
      : { ...photographedPlan, budget: budgetLedger.value.totals }
...
<template>
  <view class="plan-page" :class="preferences.themeClass" :style="mobileHeaderStyle">
    ...
    <!-- #endif -->
    <!-- #ifdef H5 -->
    <view class="plan-h5-mobile-header">
      <button class="plan-h5-mobile-action" :aria-label="t('navigation.openMenu')" @click="drawerOpen = true">
        <wd-icon name="menu" size="22px" />
      </button>
      <text class="plan-h5-mobile-brand">{{ t('app.brand') }}</text>
      <button class="plan-h5-mobile-action plan-h5-mobile-action-accent" :aria-label="t('sidebar.newPlan')" @click="goNewPlan">
        <text class="plan-h5-mobile-add">+</text>
      </button>
    </view>
    <!-- #endif -->
...
.plan-content {
  ...
  min-width: 0;
  height: 100%;
  margin-bottom: var(--plan-edit-dock-clearance);
  overflow: auto;
  flex: 1;
}
.plan-mobile-header {
...
@media (max-width: 768px) {
  .plan-page {
    --plan-edit-dock-clearance: calc(64px + max(12px, env(safe-area-inset-bottom)));
  ...
  .plan-h5-mobile-header {
    ...
    flex: 0 0 calc(52px + var(--mobile-safe-top));
  ...
  .plan-h5-mobile-action::after {
    display: none;
  }
  .plan-h5-mobile-action-accent {
