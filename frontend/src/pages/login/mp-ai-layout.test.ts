...
describe('mini-program AI login layout', () => {
  ...
  it('clips the SVG logo inside its visual frame on WeChat', () => {
    expect(source).toMatch(/\.mini-brand-logo-frame\s*\{[\s\S]*?overflow:\s*hidden/)
    expect(source).toMatch(/\.mini-brand-logo\s*\{[\s\S]*?width:\s*100%/)
    expect(source).toMatch(/\.mini-brand-logo\s*\{[\s\S]*?height:\s*100%/)
  })
  ...
  it('matches the left-aligned conversational layout selected in concept B', () => {
    expect(source).toMatch(/\.login-main\s*\{[\s\S]*?align-items:\s*flex-start/)
    expect(source).toMatch(/\.login-main\s*\{[\s\S]*?text-align:\s*left/)
    expect(source).toMatch(/\.mini-login-button\s*\{[\s\S]*?margin-top:\s*auto/)
    expect(source).toMatch(/\.mini-agent-callout\s*\{[\s\S]*?border-left:\s*6rpx solid var\(--accent-primary\)/)
  })
