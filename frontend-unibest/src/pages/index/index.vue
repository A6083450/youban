...
import { streamTripParse } from '@/services/chat-stream'
...
function refreshSuggestions(): void {
  visibleSuggestions.value = pickSuggestionBatch(suggestions.value, 5)
}
...
function pushItem(item: Omit<ChatItem, 'id'>): ChatItem {
  const created = { id: newIdentifier('message'), ...item }
...
async function handleParse(text: string): Promise<void> {
  ...
  placeholder.text = decision.text
...
async function handleDraftReply(text: string): Promise<void> {
  if (!pendingDraft.value)
    return
  const response = await confirmTripReply(text, pendingDraft.value, preferences.locale, history(), readinessToken.value)
  if (response.message)
    pushItem({ role: 'assistant', kind: 'text', text: response.message })
  if (response.action === 'confirm' && response.execution_token) {
    const confirmed = response.trip || pendingDraft.value
  ...
  <view class="planning-shell" :class="preferences.themeClass">
    ...
    <main class="chat-home">
          ...
          >
            ...
            <view class="message-column" :class="item.role">
              ...
              <view class="message-bubble" :class="[item.role, item.kind]">
                <text class="message-text">{{ item.text || (item.kind === 'streaming' ? t('result.agent.typing') : '') }}</text>
                ...
                <view v-if="item.kind === 'progress'" class="progress-block">
                  <view class="progress-track">
                    <view class="progress-value" :style="{ width: `${item.progress || 5}%` }" />
                  </view>
                  <text class="progress-meta">{{ item.progress || 5 }}%</text>
                </view>
      ...
      <view class="chat-input-area" :class="{ empty: !hasMessages }">
        ...
        <view class="composer" :class="{ disabled: busy }">
          <textarea
            v-model="input"
        ...
        <view v-if="!hasMessages" class="suggestions">
          <button
          ...
          <button class="suggestion-refresh" @click="refreshSuggestions">
            ↻ {{ t('chatHome.refreshSuggestions') }}
          </button>
        </view>
      </view>
    </main>
