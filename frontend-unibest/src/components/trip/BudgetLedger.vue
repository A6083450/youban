...
function sourceLabel(item: BudgetLedgerItemDto): string {
  ...
  if (item.price_source === 'user')
    return t('result.budget.userPriceWithSource', { provider: item.entity_source })
  return item.price_provider || item.entity_source
}
    ...
    <aside class="right-budget-summary">
      <view class="budget-summary-panel">
        ...
        <view class="budget-summary-total-wrap">
          ...
          <text class="budget-summary-total-value">{{ totals.total.toLocaleString('zh-CN', { maximumFractionDigits: 0 }) }}</text>
