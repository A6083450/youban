...
function isCustomTabbarItem(item: CustomTabBarItem | NativeTabBarItem): item is CustomTabBarItem {
  return 'iconType' in item && 'icon' in item
}
...
function normalizeTabbarItem(item: CustomTabBarItem | NativeTabBarItem): CustomTabBarItem {
  ...
  return {
    ...
    iconType: 'image',
    icon: item.iconPath ?? '',
    iconActive: item.selectedIconPath,
  }
}

const baseTabbarList = reactive<CustomTabBarItem[]>(_tabbarList.map(normalizeTabbarItem))
