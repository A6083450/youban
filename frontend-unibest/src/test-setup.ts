...
const uniMock = {
  ...
  request: vi.fn(),
  uploadFile: vi.fn(),
  login: vi.fn((options: { success?: (result: { code: string }) => void }) => options.success?.({ code: 'wx-code' })),
  chooseImage: vi.fn(),
  getLocale: vi.fn().mockReturnValue('zh-Hans'),
  getSystemInfoSync: vi.fn().mockReturnValue({ platform: 'devtools' }),
  getSystemInfo: vi.fn(),
  getAccountInfoSync: vi.fn().mockReturnValue({ miniProgram: { envVersion: 'develop' } }),
