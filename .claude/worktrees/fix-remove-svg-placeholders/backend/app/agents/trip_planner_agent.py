...
class MultiAgentTripPlanner:
    ...
    async def _emit_progress(
        ...
        if details is not None:
            result = progress_callback(stage, message, progress, details)
        else:
        ...
        try:
            ...
            for idx, city_stay in enumerate(cities):
                ...
                await self._emit_progress(
                    ...
                    details=[{
                        "type": "searching",
                        "title": f"🔍 正在搜索 {city} 的{keywords}景点...",
                        "content": f"使用高德地图搜索 {city} 的 {keywords} 相关景点信息",
                ...
                try:
                    import json as _json
                    _parsed = _json.loads(attraction_response) if isinstance(attraction_response, str) else attraction_response
                    _count = len(_parsed) if isinstance(_parsed, list) else 0
                except Exception:
                    _count = 0
                await self._emit_progress(
                    progress_callback, "attraction_search",
                    f"✅ {city} 景点搜索完毕，找到 {_count} 个景点",
                    progress_base,
                    details=[{
                        "type": "found",
                        "title": f"📍 {city} 景点搜索完成",
                        "content": f"找到 {_count} 个{keywords}相关景点" if _count else f"已完成 {city} 景点搜索",
                        "timestamp": int(_time.time() * 1000),
                    }],
                )
                ...
                await self._emit_progress(
                    ...
                    f"🌤️ 正在查询 {city} 的天气...{city_label}",
                    progress_base + progress_step,
                    details=[{
                        "type": "searching",
                        "title": f"🌤️ 正在查询 {city} 未来天气预报...",
                        "content": f"调用天气 API 获取 {city} 的天气预报数据",
                        "timestamp": int(_time.time() * 1000),
                    }],
                )
                ...
                await self._emit_progress(
                    ...
                    details=[{
                        "type": "found",
                        "title": f"🌤️ {city} 天气查询完成",
                        "content": weather_response[:200] if weather_response else "天气数据获取成功",
                        "timestamp": int(_time.time() * 1000),
                    }],
                )
                ...
                await self._emit_progress(
                    ...
                    details=[{
                        "type": "searching",
                        "title": f"🏨 正在搜索 {city} 的{request.accommodation}酒店...",
                        "content": f"根据住宿偏好「{request.accommodation}」搜索 {city} 合适的酒店",
                        "timestamp": int(_time.time() * 1000),
                    }],
                )
                ...
                await self._emit_progress(
                    ...
                    details=[{
                        "type": "found",
                        "title": f"🏨 {city} 酒店搜索完成",
                        "content": hotel_response[:200] if hotel_response else "酒店搜索完成",
                        "timestamp": int(_time.time() * 1000),
                    }],
                )
            ...
            import time as _time
            await self._emit_progress(
                progress_callback, "planning", planning_label, 85,
                details=[{
                    "type": "planning",
                    "title": f"🧠 正在综合分析 {city_display} 的景点、天气和酒店信息...",
                    "content": f"已收集 {total_cities} 个城市的基础数据，AI 正在规划最优行程路线",
                    "timestamp": int(_time.time() * 1000),
                }],
            )
