...
Fix round 1 moved the generation start marker to the server acceptance boundary. The marker is kept only on the active run, refreshed on retry, and passed into the coordinator so startup delay reduces the remaining fast-plan budget. It also replaced the visibility-off browser mock with a real in-process HTTP runtime whose assistant attempts the known intake-stage summary while visibility is disabled.

Fix round 2 made both acceptance claims auditable. The visibility browser flow now forwards the frontend's real conversation create, update, detail, and list requests to that runtime, proves a non-empty persisted conversation containing the normal user and assistant messages, and then checks the stored snapshot for absence of thinking content. The browser measurements are generated as machine-readable JSON from the same live browser timestamps and raw status responses shown on the recorded evidence page.
...
Fix round 2 recording: `/Users/liangjiaquan/.config/browser-harness/agent-workspace/recordings/task8-planning-deadlines-fix2` (26 frames)

Machine-readable evidence: `docs/superpowers/reports/2026-08-24-thinking-controls-and-planning-deadlines-measurements.json`

Evidence-page screenshot: `/Users/liangjiaquan/.config/browser-harness/agent-workspace/recordings/task8-planning-deadlines-fix2/measurements.png`

The fix-round browsers used task-owned services on `127.0.0.1:8193` and `127.0.0.1:8194`. A temporary, test-only route outside the repository issued a confirmation token for a fixed, already-confirmed draft; no test route was added to the production application. Runtime provider settings were copied into temporary data directories without printing credentials. The Browser Use tab was activated for the final measurements because Chrome throttled polling in a hidden tab.
...
Fix round 2 raw rerun:

| Days | Task | Browser POST to readable DOM | Limit | Server accepted to persisted | First quality | First state | Day count | Terminal elapsed | Terminal state |
| ---: | --- | ---: | ---: | ---: | --- | --- | ---: | ---: | --- |
| 7 | `ada0dee3` | 5,696 ms | 6,000 ms | 5,505 ms | fast | running | 7 | 14,497 ms | fast / skipped |
| 15 | `8ba9ca85` | 9,662 ms | 10,000 ms | 9,504 ms | fast | running | 15 | 16,799 ms | fast / skipped |
| 30 | `eacda751` | 14,703 ms | 15,000 ms | 14,506 ms | fast | running | 30 | 16,124 ms | enhanced / completed |
...
- Settings persistence, real-backend visibility-off behavior, fast-plan navigation, all three terminal notices, edit-conflict preservation, and raw-evidence validation passed in `frontend/e2e/planning-deadlines.spec.ts` (8/8).
- The visibility-off test forwards the browser request and all frontend conversation persistence requests to a real in-process HTTP runtime configured with `thinkingVisible: false`. The assistant attempts the deterministic intake-stage summary. The test proves the real store contains one revisioned conversation with the normal user and assistant messages, while the captured upstream SSE, visible DOM, and persisted detail contain no thinking event or summary.
...
- Persistence RED: after requiring one real stored session, the prior browser route returned the mocked empty conversation list and failed with `Expected: 1, Received: 0`.
- Persistence GREEN: conversation create, update, detail, and list calls now reach the same real fixture. The focused visibility test passed with a revisioned, non-empty user/assistant snapshot and no thinking content in SSE, DOM, or storage.
- Raw-evidence RED: the new audit test failed with `ENOENT` before the measurement JSON existed.
- Raw-evidence GREEN: Browser Use generated the JSON from three live cases, displayed the identical payload in the evidence DOM, captured it in the recording and screenshot, and the focused Playwright file passed 8/8.
...
| `cd frontend && bunx playwright test e2e/planning-deadlines.spec.ts --workers=1` | PASS: 8 tests |
| `git diff --check` | PASS |
...
Task-owned ports `8191`, `8192`, `8193`, and fix-round-2 port `8194` were stopped after evidence collection. The fix-round temporary data directories were moved to the system Trash so their provider-settings copies were not left in `/tmp`. The pre-existing process on port `7860` was not stopped or replaced. Only tabs created for this task were closed.
