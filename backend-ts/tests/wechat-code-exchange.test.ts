...
describe("WechatCodeExchange", () => {
  it("exchanges a code for the openid without returning session_key", async () => {
    let requested = "";
    const exchange = new WechatCodeExchange({
      ...
      fetch: async (input) => {
        ...
        return Response.json({ openid: "openid-1", session_key: "session-secret" });
      },
    });
    expect(await exchange.exchange("temporary-code")).toBe("openid-1");
