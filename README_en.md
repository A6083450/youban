...
> [!IMPORTANT]
> 
> You can deploy the project locally to try it out directly. For the full feature experience, make sure the relevant API keys are configured: [YouBan - AI Travel Agent](https://modelscope.cn/studios/lcclxy/Journey-to-the-China)
> 
...
**YouBan** is an innovative AI travel agent application, a multi-agent collaborative travel planning platform based on the HelloAgents framework, designed to solve the "information overload" and "decision fatigue" users face when planning trips.
...
* **Amap Data-Driven**: Attraction recommendations and image data come from real POI data on Amap (GaoDe Maps). Attraction images are automatically downloaded and cached to the local `data` directory.
* **Attraction Reservation Reminders**: Intelligently identifies attractions that require advance booking (e.g., the Forbidden City, Shaanxi History Museum), distinctly highlighting reservation prompts and channels in the itinerary cards to save you a wasted trip.
...
graph TD
    ...
    subgraph G3 ["Multi-Agent Engine"]
        ...
        C2["Amap POI Parser<br/>(POI Search + LLM Refine)"]
        C3["Weather Agent"]
    ...
    subgraph G4 ["Service Layer"]
        ...
        D4["Amap POI API<br/>Search/Photos"]
    end
...
1. **Amap Attraction Extraction**: Fetches popular attraction candidates via the Amap POI search API, then uses the LLM to refine them into structured data: attraction names, authentic reviews, play durations, and reservation requirements, with precise coordinates filled in.
...
4. **Attraction Image Search (Frontend Driven)**: After generating the itinerary, the frontend independently calls the `/api/poi/photo` endpoint for each attraction. The backend searches attraction photos via Amap and automatically downloads & caches them into the local `data/images` directory, ensuring long-term stable access.
...
# Install dependencies with uv (auto-creates .venv and installs from uv.lock)
uv sync

# Copy config and fill your API KEYs
cp .env.example .env
# [Required] LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_ID (pick a model good at JSON struct)
# [Optional] VITE_AMAP_WEB_KEY (attraction search & photos)

# Start FastAPI (Uvicorn recommended; uv run uses .venv automatically)
...
│   │   │   ├── amap_service.py   # Amap POI search / photo cache
...
- [x] ~~Amap POI data integration for high-quality plans~~
- [x] ~~Attraction images via Amap with local caching~~
...
## 🙏 Acknowledgements
Thanks to the [linux.do](https://linux.do/) community for communication and feedback, making YouBan iterate more efficiently.
