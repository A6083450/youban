...
describe("trip planning HTTP lifecycle", () => {
  ...
  it("allows only an administrator to fully regenerate a completed task", async () => {
    const { runtime, planner } = makeRuntime();
    const token = runtime.assistant.ledger.register(DRAFT, 0.95).token;
    const accepted = await (await request(runtime.app, "POST", "/api/trip/plan", {
      ...DRAFT,
    ...
    const ownerRetry = await request(runtime.app, "POST", `/api/trip/plan/${accepted.task_id}/retry`, {
      restart_all: true,
    });
    expect(ownerRetry.status).toBe(409);

    const adminWithoutReset = await runtime.app.handle(new Request(
      `http://localhost/api/trip/plan/${accepted.task_id}/retry`,
      {
        method: "POST",
        headers: { "content-type": "application/json", "x-admin-token": "admin@123" },
        body: JSON.stringify({ restart_all: false }),
      },
    ));
    expect(adminWithoutReset.status).toBe(409);

    const adminRetry = await runtime.app.handle(new Request(
      `http://localhost/api/trip/plan/${accepted.task_id}/retry`,
      {
        method: "POST",
        headers: { "content-type": "application/json", "x-admin-token": "admin@123" },
        body: JSON.stringify({ restart_all: true }),
      },
    ));
    expect(adminRetry.status).toBe(200);
    expect(await adminRetry.json()).toEqual(expect.objectContaining({
      task_id: accepted.task_id,
      status: "processing",
    }));
    await waitFor(() => runtime.tasks.get(accepted.task_id)?.status === "completed");
    expect(planner.runs).toHaveLength(2);
    expect(planner.runs[1]?.checkpoint).toEqual(emptyCheckpoint());
  });
});
