...
describe("authentication schema migration", () => {
  it("creates a fresh schema with isolated nickname and WeChat identities", () => {
    ...
    expect((database.raw.query("PRAGMA user_version").get() as { user_version: number }).user_version).toBe(8);
    ...
    const nicknameIdentityColumns = database.raw.query(
      "PRAGMA table_info(nickname_identities)",
    ).all() as Array<{ name: string }>;
    expect(nicknameIdentityColumns.map(column => column.name).sort()).toEqual([
      "created_at",
      "last_login_at",
      "nickname_digest",
      "user_id",
  ...
  it("upgrades a version 6 database without replacing existing tables", () => {
    ...
    expect(database.raw.query(
      "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'nickname_identities'",
    ).get()).toEqual({ name: "nickname_identities" });
    database.close();
  });
});
