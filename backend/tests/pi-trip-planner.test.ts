...
describe("PiTripPlanner", () => {
  ...
  it("tops up a sparse research selection to cover each destination day", async () => {
    const tenPois = Array.from({ length: 10 }, (_, index) => ({
      poi_id: `T${index + 1}`,
      name: `草原景点${index + 1}`,
      address: `地址${index + 1}`,
      type: "风景名胜",
      location: { longitude: 117 + index / 100, latitude: 42 + index / 100 },
    }));
    class DenseResearch extends FakeResearch {
      override async searchAttractions(city: string) {
        this.calls.push(`attractions:${city}`);
    ...
    class SparseSelectionAgents extends FakeAgents {
      override async run(request: StructuredAgentRequest): Promise<unknown> {
        if (request.agent === "destination-researcher") {
          const candidates = (request.input as Record<string, any>).candidates as Array<Record<string, any>>;
          return { selected_poi_ids: candidates.slice(0, 2).map((candidate) => candidate.poi_id) };
        }
        return super.run(request);
      }
    }
    const planner = new PiTripPlanner({
      research: new DenseResearch(),
      agents: new SparseSelectionAgents(),
    });
    const tenDayRequest = {
      ...REQUEST,
      city: "乌兰布统草原",
      cities: [{ city: "乌兰布统草原", days: 10 }],
      end_date: "2026-10-10",
      travel_days: 10,
    };

    const result = await planner.plan(tenDayRequest, context().value);
    const days = (result.data as Record<string, any>).days as Array<Record<string, any>>;
    expect(days).toHaveLength(10);
    expect(days.every((day) => day.attractions.length >= 1)).toBeTrue();
    expect(new Set(days.flatMap((day) => day.attractions.map((item: Record<string, any>) => item.poi_id))).size)
      .toBe(10);
  });
