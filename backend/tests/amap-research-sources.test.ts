...
describe("AmapResearchSources", () => {
  ...
  it("uses a generic hotel query when accommodation is unspecified", async () => {
    const placeQueries: string[] = [];
    const sources = new AmapResearchSources({
      apiKey: "web-key",
      minimumRequestIntervalMs: 0,
      fetch: async (input) => {
        const url = new URL(String(input));
        if (url.pathname === "/v3/config/district") {
          return jsonResponse({ status: "1", districts: [{ name: "克什克腾旗", adcode: "150425" }] });
        }
        placeQueries.push(url.searchParams.get("keywords") ?? "");
        return jsonResponse({ status: "1", pois: [] });
      },
    });

    await sources.searchHotels("乌兰布统草原", "未指定");
    expect(placeQueries).toEqual(["酒店"]);
  });
  ...
  it("resolves 乌兰布统草原 through its searchable destination alias", async () => {
    const urls: URL[] = [];
    const sources = new AmapResearchSources({
      apiKey: "web-key",
      minimumRequestIntervalMs: 0,
      fetch: async (input) => {
        ...
        if (url.pathname === "/v3/config/district") {
          ...
          return jsonResponse({
            status: "1",
            districts: keyword === "乌兰布统"
              ? [{ name: "克什克腾旗", adcode: "150425" }]
              : [],
          });
        }
        return jsonResponse({
          status: "1",
          pois: [{ id: "W001", name: "乌兰布统景区", location: "117.38,42.49", type: "风景名胜" }],
        });
      },
    });

    expect(await sources.searchAttractions("乌兰布统草原", ["草原风光"])).toEqual([
      expect.objectContaining({ poi_id: "W001", name: "乌兰布统景区" }),
    ]);
    expect(urls[0]?.searchParams.get("keywords")).toBe("乌兰布统");
    expect(urls.at(-1)?.searchParams.get("region")).toBe("150425");
  });
