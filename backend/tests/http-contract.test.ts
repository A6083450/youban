...
describe("HTTP compatibility contract", () => {
  ...
  it("serves the uni-app shell and retires legacy PWA clients", async () => {
    const { app } = runtime({ frontend: true });
    const root = await app.handle(new Request("http://localhost/"));
    expect(root.status).toBe(200);
    expect(root.headers.get("cache-control")).toBe("no-cache");
    const retiredWorker = await app.handle(new Request("http://localhost/sw.js"));
    expect(retiredWorker.status).toBe(200);
    expect(await retiredWorker.text()).toBe("service-worker");
    expect(retiredWorker.headers.get("cache-control")).toBe("no-store");
    expect(retiredWorker.headers.get("service-worker-allowed")).toBe("/");
    for (const path of ["/registerSW.js", "/manifest.webmanifest"]) {
