...
export const nicknameIdentitiesTable = sqliteTable(
  "nickname_identities",
  {
    nicknameDigest: text("nickname_digest").primaryKey(),
    userId: text("user_id").notNull().references(() => usersTable.userId, { onDelete: "cascade" }),
    createdAt: text("created_at").notNull(),
    lastLoginAt: text("last_login_at").notNull(),
  },
  (table) => [uniqueIndex("nickname_identities_user_idx").on(table.userId)],
);
...
export const schema = {
  ...
  nicknameIdentities: nicknameIdentitiesTable,
...
export const NICKNAME_AUTH_SCHEMA_SQL = `
  CREATE TABLE IF NOT EXISTS nickname_identities (
    nickname_digest TEXT PRIMARY KEY NOT NULL,
    user_id TEXT NOT NULL UNIQUE REFERENCES users(user_id) ON DELETE CASCADE,
    created_at TEXT NOT NULL,
    last_login_at TEXT NOT NULL
  );
  CREATE UNIQUE INDEX IF NOT EXISTS nickname_identities_user_idx
    ON nickname_identities(user_id);
`;
