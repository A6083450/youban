...
const NO_STORE_FRONTEND_FILES = new Set([
  "sw.js",
]);
...
function frontendResponse(frontendDist: string | undefined, pathname: string): Response | null {
  ...
  if (NO_STORE_FRONTEND_FILES.has(relative)) headers.set("Cache-Control", "no-store");
  if (relative === "sw.js") headers.set("Service-Worker-Allowed", "/");
...
export function createHttpRuntime(options: HttpRuntimeOptions) {
  ...
  const skills = options.skillService ?? new SkillManagementService({
    ...
    builtinSkillsDir: join(repoRoot, "backend", "src", "agents", "skills"),
  });
  ...
  const app = new Elysia({ name: "youban-http" })
    ...
    .post("/api/auth/nickname", ({ body, set, status }) => {
      if (!authentication) return status(503, { detail: "认证服务尚未配置" });
      try {
        const result = authentication.loginNicknameIdentity(body.nickname);
        set.headers["Cache-Control"] = "no-store";
        set.headers["Set-Cookie"] = webSessionCookie(result.token);
      ...
      body: NicknameLoginBodySchema,
      response: {
        200: AuthResponseSchema,
        422: DetailErrorSchema,
        503: DetailErrorSchema,
      },
    })
    ...
    .post("/api/trip/plan/:planId/retry", ({ params, body, headers, status }) => {
      ...
      if (ownerError) return status(ownerError.status, { detail: ownerError.detail });
      const adminFullRegeneration = task.status === "completed"
        && body.restart_all === true
        && validAdminToken(headers);
      if (task.status !== "failed" && !adminFullRegeneration) {
        return status(409, { detail: "仅失败任务可重试" });
      }
