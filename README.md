...
**游伴 (YouBan)** 是一个创新的 AI 文旅智能体应用，基于 HelloAgents 框架打造的多智能体协作文旅规划平台，旨在解决用户在规划旅行时面临的"信息过载"和"决策疲劳"问题。
...
* **高德数据驱动**: 景点推荐与图片数据来源于高德地图真实 POI 数据，景点图片自动下载缓存到本地 data 目录。
* **景点预约提醒**: 智能识别需要提前预约的景点（如故宫、陕西历史博物馆等），在行程卡片中醒目标注预约提示与预约渠道信息，防止白跑一趟。
...
sequenceDiagram
    ...
    rect rgb(240, 248, 255)
        ...
        par [1/3] 景点搜索：高德 POI 搜索 + LLM 提纯
            Planner->>AMap: search_amap_attractions(city, keywords, lang)
            AMap->>AMap: 高德 POI 文本搜索（风景名胜类型，限定城市）
            AMap->>LLM: 抛入 POI 候选数据，Prompt 要求提纯出景点JSON数组
            LLM-->>AMap: [{"name": "故宫", "duration": 120, ...}]

            loop 为每个提纯出的景点补齐坐标
                AMap->>Maps: geocode_unified(name, city)
                Note right of Maps: Google 地理编码优先，失败降级高德 REST
                Maps-->>AMap: 经纬度 {longitude, latitude}
            end
            AMap-->>Planner: 拼接整理好的高德景点候选文本
    ...
    rect rgb(240, 255, 240)
        Note over Client, AMap: 异步前端懒加载：景点图片搜图
        Client->>POI: GET /api/poi/photo?name=xxx
        POI->>AMap: get_cached_poi_photo(name, city)
        AMap->>AMap: 高德搜索景点图片，下载缓存到本地 data/images 目录
        AMap-->>POI: photo_url
        POI-->>Client: 图片加载成功
    end
```
...
1. **高德景点提取**: 通过高德 POI 搜索接口获取城市热门景点候选，再由 LLM 从候选数据中提纯出景点名称、真实评价、游玩时长以及是否需要提前预约等结构化信息，并补齐精准经纬度坐标。
2. **天气与酒店**: 天气管家查询目标日期的气候状况；酒店专员根据预算寻找合适落脚点。
3. **路线编排**: 主控 Agent 收集三方数据，进行统筹优化，计算两两景点间的距离和最优游玩顺序，避免行程折返跑。
4. **景点搜图 (前端驱动)**: 行程生成完毕后，前端根据每个景点名称独立调用 `/api/poi/photo` 接口，后端通过高德搜索景点图片并自动下载缓存到本地 data/images 目录，确保图片可长期稳定访问。
...
# 使用 uv 安装依赖（自动创建 .venv 并按 uv.lock 精确安装）
uv sync
...
# 启动 FastAPI（uv run 自动使用 .venv，无需手动激活）
...
│   │   ├── services/              # 业务逻辑封装
│   │   │   ├── amap_service.py    # 高德 POI 搜索/LLM提纯/景点图片缓存
...
## 🙏 致谢
感谢 [linuxdo](https://linux.do/) 社区的交流、分享与反馈，让游伴的迭代更高效，同时欢迎大家进群交流反馈
